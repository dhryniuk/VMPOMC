...
function one_body_Lindblad_term(params::parameters, sample::density_matrix, j::UInt16, l1::Matrix, A::Array{ComplexF64,3}, L_set::Vector{Matrix{ComplexF64}}, R_set::Vector{Matrix{ComplexF64}})
    ...
    @inbounds for (i::UInt16,state::Tuple{Bool,Bool}) in zip(1:4,TPSC::Vector{Tuple{Bool,Bool}})
...
function one_body_Lindblad_term(params::parameters, sample_ket::Array{Bool}, sample_bra::Array{Bool}, j::UInt16, l1::Matrix, A::Array, L_set::Vector{Matrix{ComplexF64}}, R_set::Vector{Matrix{ComplexF64}})
    local_L::ComplexF64 = 0
    local_∇L::Array{ComplexF64,3}=zeros(ComplexF64,params.χ,params.χ,4)

    s = dVEC[(sample_ket[j],sample_bra[j])]
    bra_L = transpose(s)*conj(l1)
    #for i in 1:4
    for (i,state) in zip(1:4,TPSC)
        loc = bra_L[i]
        if loc!=0
            #state = TPSC[i]
            #local_L += loc*tr(L_set[j]*A[:,:,dINDEX[(state[1],state[2])]]*R_set[params.N+1-j])
            local_L += loc*tr(L_set[j]*A[:,:,i]*R_set[params.N+1-j])
            micro_sample_ket = deepcopy(sample_ket)
            micro_sample_bra = deepcopy(sample_bra)
            micro_sample_ket[j] = state[1]
            micro_sample_bra[j] = state[2]
            
            micro_L_set = L_MPO_strings(params, micro_sample_ket, micro_sample_bra, A)
            micro_R_set = R_MPO_strings(params, micro_sample_ket, micro_sample_bra, A)
            local_∇L+= loc*derv_MPO(params, micro_sample_ket, micro_sample_bra, micro_L_set, micro_R_set)
        end
    end
    return local_L, local_∇L
end
...
function Exact_MPO_gradient(params::parameters, A::Array{ComplexF64}, l1::Matrix{ComplexF64}, basis)
