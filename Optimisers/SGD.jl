...
function calculate_MPS_gradient(params::parameters, A::Array{ComplexF64}, basis)
    ...
    for k in 1:params.dim
        ...
        #Δ_MPO_sample = derv_MPS(params, sample, A)/ρ_sample
        Δ_MPO_sample = conj( derv_MPS(params, sample, L_set, R_set) /ρ_sample )
        #println()
        #println(sample)
        #println("Gradient:")
        #display(Δ_MPO_sample)

        #Add in interaction terms:
        local_E  = e_int+e_field
        #local_∇E = (e_int+e_field)*Δ_MPO_sample
        L∇L += p_sample*Δ_MPO_sample*local_E # conj?
        #println(p_sample)
        #println(e_int+e_field)
        #display(p_sample*Δ_MPO_sample*local_E)
        #display(L∇L)
