...
function one_body_Hamiltonian_term(params::parameters, sample::Vector{Bool}, j::UInt16, h1::Matrix, A::Array, L_set::Vector{Matrix{Float64}}, R_set::Vector{Matrix{Float64}})
    energy::Float64 = 0
    s = dVEC2[sample[j]]
    bra_L = transpose(s)*h1
    for (i,state,k) in zip(1:2,[true,false],2:-1:1)
        loc = bra_L[i]
        if loc!=0
            #state = TPSC2[i]
