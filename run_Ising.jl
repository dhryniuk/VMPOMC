...
γ_d = 0.0#parse(Float64,ARGS[2])
hx = 0.5#parse(Float64,ARGS[3])
...
δ::Float64 = 0.003
F::Float64=0.998
ϵ::Float64=parse(Float64,ARGS[3])#0.01


sampler = MetropolisSampler(10*χ^2, 0)
#optimizer = SR(sampler, A, l1, ϵ, params, "Ising", "Local")
optimizer = SGD(sampler, A, l1, params, "Ising", "Local")
N_iterations = 1000
...
@time begin
    for k in 1:N_iterations
        for i in 1:1#10
            ...
            #println(optimizer.optimizer_cache.mlL)
            #sleep(10*mpi_cache.rank)
...
if mpi_cache.rank == 0
    ...
    o = open("Ising_decay_chi$(χ)_N$(N)_hx$(hx).out", "a")
    println(o,"Elapsed time: $elapsed_time seconds")
    close(o)
end