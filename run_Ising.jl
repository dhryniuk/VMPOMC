...
for k in last_iteration_step:N_iterations
    ...
    if mpi_cache.rank == 0
        ...
        A_dagger = reshape(Af_dagger,χ,χ,4) 

        #ρ = compute_density_matrix(params,A,basis)
        #display(ρ)
        #ρ = compute_density_matrix(params,A_dagger,basis)
        #display(ρ)

        #A_herm = hermetize_MPO(params,optimizer.A)
        #A_herm = normalize_MPO!(params, A_herm)
        #ρ = compute_density_matrix(params,A_herm,basis)
        #display(ρ)
        #display(adjoint(ρ)==ρ)
        #sleep(1)
        #error()
        ...
        close(list_of_mag)
        list_of_P = open("list_of_P.data", "a")
        P = real(tensor_purity(params, optimizer.A))
        println(list_of_P, P)
        close(list_of_P)
        list_of_cor = open("list_of_cor.data", "a")
